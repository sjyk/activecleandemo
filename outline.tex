...
\title{ActiveClean:  Interactive Data Cleaning For Modern Machine Learning}

\numberofauthors{1}
\author{\large Sanjay Krishnan{$\,^*$}, Michael J. Franklin{$\,^*$}, Ken Goldberg{$\,^*$}, Eugene Wu{$\,^\dag$}\\
\vspace{.2em}\affaddr{\large UC Berkeley$\,^*$, ~~ $^\dag$Columbia University} \\
\vspace{.1em}\affaddr{\large \{sanjaykrishnan, franklin, goldberg\}@berkeley.edu, ewu@cs.columbia.edu}\\
}
...
We propose \sysfull, an anytime framework for training Machine Learning models with budgeted data cleaning.
...
In this demonstration, we designed a visual interface to wrap around this framework and demonstrate \sys for a video classification problem and a topic modeling problem. 
