...
\ewu{COMPLETELY UNNECESSARY.  CUT! Once the model is instantiated, she can train the model on the dirty data.
...
When she opens the \textsf{Diagnose} panel, \sys applies the importance sampling algorithm \ewu{Rename estimate-driven prioritization or establish a consistent name with Section 2} to select and visualize a subset of examples from the dataset.
...
\ewu{Maybe cut the rest.. What does this sentence mean? How does drawing not quite cleaned data (whatever tha means) translate into an optimization?} 
