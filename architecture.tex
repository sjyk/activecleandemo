...
\ewu{To summarize, prior work studies how to \emph{use machine learning models} to improve data cleaning.
In contrast, \sys explores how to control the impact of data cleaning \emph{on downstream machine learning models}.
...
It also leads to optimization opportunities -- how the data cleaning step be made aware of the subsequent data analysis (e.g., the model that is trained) 
...
One of the primary contributions of this work is an incremental model update algorithm with correctness guarantees for the \ewu{resulting mixture of dirty and clean} data.
...
This work focuses on a class of well-analyzed predictive analytics problems
\ewu{-- ones that can be expressed as the minimization of loss functions -- that will be trained on the output of applying $F(\cdot)$ to the records in $R$.}
...
We note that designing \sys for this form of machine learning model enables \sys to work for \ewu{SVMs, logistic regression, Latent Dirchelet Analysis, whatever... Sanjay fill in popular types of models you want to name drop}.

\subsection{Required User Input}\label{uinp}
\ewu{In such a problem setting, \sys takes as input four user-defined functions.  The first two are used in order to train models, and can be assumed to be readily available.  
The latter two consist of a user tunable parameters with reasonable defaults,
and a generic record cleaning function:}
...
\ewu{This formulation supports common operations such as extraction, deletion, transformation, etc etc. FILL IN}
...
the sample to the {\it Cleaner}, which executes $C(\cdot)$ for each sample record \ewu{or on the whole sample?  given map/filter formulation?} and outputs their cleaned versions.
...
\ewu{Sentence about which of the functions are hard and are contributions.  }


\subsection{Optimizations}
\subsection{Technical Details}
We overview some of the research contributions and the details can be found in~\cite{activecleanarxiv}.
\ewu{Tie points below back to the functions in S2.4.  E.g., Festimate-Driven Prioritiation: this is an optimization on the Sampler that ....  Or explicitly change the item heading to Sampler/Updater/Estimator/etc }
