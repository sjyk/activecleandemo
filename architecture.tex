...
\sys borrows ideas from active learning in terms of improving the efficiency of expensive data cleaning and taking an incremental approach.
\ewu{This is still confusing why it's different from active learning (the downsream business).  Maybe just focus on correctness and how points are pickd?}  However, \sys addresses a very different problem, namely, how information about a user's subsequent data analysis (the model that is trained) can inform which data to clean.
